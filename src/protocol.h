...
enum ServiceFlags : uint64_t {
    ...
    // chain. It is currently set by all BitFinite Node non pruned nodes, and is
    // unset by SPV clients or other light clients.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo
    // protocol request. BitFinite Node does not support this but a patch set
    ...
    // connections. BitFinite Node nodes used to support this by default, without
    ...
    NODE_XTHIN = (1 << 4),
    // NODE_BITCOIN_CASH means the node supports BitFinite and the
    // associated consensus rule changes.
    // This service bit is intended to be used prior until some time after the
    // UAHF activation when the BitFinite network has adequately separated.
...
public:
    ...
    CAddress(CService ipIn, ServiceFlags nServicesIn, uint32_t nTimeIn)
        : CService{ipIn}, nServices{nServicesIn}, nTime{nTimeIn} {}
    ...
    SERIALIZE_METHODS(CAddress, obj) {
        ...
        if ((s.GetType() & SER_DISK) || (nVersion != INIT_PROTO_VERSION && !(s.GetType() & SER_GETHASH))) {
            ...
            // uses INIT_PROTO_VERSION. Currently extversion in BFXN does not
