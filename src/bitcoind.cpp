 ...
 *
 * This is the developer documentation of BitFinite Node
 * (https://www.bitfinitenode.org/). BitFinite Node is a client for the
 * digital currency called BitFinite (https://www.bitfinite.org/), which
 * enables instant payments to anyone, anywhere in the world. BitFinite uses
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr,
                        ...
                        "see bitfinited -h for a list of options.\n",
                        argv[i]);
                return false;
            }
        }

        // -server defaults to true for bitfinited but not for the GUI so do this
        // here
...
int main(int argc, char *argv[]) {
    ...
    // Connect bitfinited signal handlers
    noui_connect();
