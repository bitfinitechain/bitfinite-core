    ...
    switch (*buffer) {
        ...
        case '9': {
            ...
            if (*buffer == '.') {
                ...
                if (!json_isdigit(*buffer)) return JTOK_ERR;
                do { // consume digits
                    ++buffer;
...
const char *UniValue::read(const char *buffer) {
    ...
    do {
        ...
        if (tok == JTOK_NONE || tok == JTOK_ERR) return nullptr;

        bool isValueOpen = jsonTokenIsValue(tok) || tok == JTOK_OBJ_OPEN || tok == JTOK_ARR_OPEN;

        if (expect(VALUE)) {
            if (!isValueOpen) return nullptr;
            clearExpect(VALUE);

        } else if (expect(ARR_VALUE)) {
            bool isArrValue = isValueOpen || (tok == JTOK_ARR_CLOSE);
            if (!isArrValue) return nullptr;

            clearExpect(ARR_VALUE);

        } else if (expect(OBJ_NAME)) {
            bool isObjName = (tok == JTOK_OBJ_CLOSE || tok == JTOK_STRING);
            if (!isObjName) return nullptr;

        } else if (expect(COLON)) {
            if (tok != JTOK_COLON) return nullptr;
            clearExpect(COLON);
        ...
        switch (tok) {
            ...
            case JTOK_ARR_OPEN: {
                ...
                if (stack.size() > MAX_JSON_DEPTH) return nullptr;
            ...
            case JTOK_OBJ_CLOSE:
            case JTOK_ARR_CLOSE: {
                if (!stack.size() || (last_tok == JTOK_COMMA)) return nullptr;

                VType utyp = (tok == JTOK_OBJ_CLOSE ? VOBJ : VARR);
                UniValue *top = stack.back();
                if (utyp != top->getType()) return nullptr;
            ...
            case JTOK_COLON: {
                if (!stack.size()) return nullptr;

                UniValue *top = stack.back();
                if (top->getType() != VOBJ) return nullptr;
            ...
            case JTOK_COMMA: {
                if (!stack.size() || (last_tok == JTOK_COMMA) || (last_tok == JTOK_ARR_OPEN)) return nullptr;
            ...
            case JTOK_STRING: {
                if (expect(OBJ_NAME)) {
                    UniValue *top = stack.back();
                    top->entries.emplace_back(std::piecewise_construct, std::forward_as_tuple(std::move(tokenVal)),
    ...
    tok = getJsonToken(tokenVal, buffer);
    if (tok != JTOK_NONE) return nullptr;
