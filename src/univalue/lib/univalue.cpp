...
void UniValue::operator=(short val_) {
    setInt64<int64_t>(val_);
}
void UniValue::operator=(int val_) {
    setInt64<int64_t>(val_);
}
void UniValue::operator=(long val_) {
    setInt64<int64_t>(val_);
}
void UniValue::operator=(long long val_) {
    setInt64<int64_t>(val_);
}
void UniValue::operator=(unsigned short val_) {
    setInt64<uint64_t>(val_);
}
void UniValue::operator=(unsigned val_) {
    setInt64<uint64_t>(val_);
}
void UniValue::operator=(unsigned long val_) {
    setInt64<uint64_t>(val_);
}
void UniValue::operator=(unsigned long long val_) {
    setInt64<uint64_t>(val_);
}
...
bool UniValue::operator==(const UniValue &other) const noexcept {
    ...
    // Returning true is the default behavior, but this is not included as a default statement inside the switch
    // statement, so that the compiler warns if some type is not explicitly listed there.
    return true;
}
