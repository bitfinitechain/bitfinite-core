...
def assert_fee_amount(fee, tx_size, fee_per_kB, wiggleroom=2):
    ...
    if fee < (tx_size - wiggleroom) * fee_per_kB / 1000:
        raise AssertionError(
            "Fee of {} BFX too low! (Should be {} BFX)".format(str(fee), str(target_fee)))
    if fee > (tx_size + wiggleroom) * fee_per_kB / 1000:
        raise AssertionError(
            "Fee of {} BFX too high! (Should be {} BFX)".format(str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting BFX values"""
