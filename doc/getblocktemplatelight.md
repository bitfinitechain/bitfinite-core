...
Comments-Summary: BFXN internal review comments.
```
...
NOTE: The first element of the Merkle path list will be a transaction hash
      ...
      on the size of the job id string, for example the BitFinite Node
...
(2) The coinbase field has an enforced minimum size of 100 bytes.
    ...
    For example, in the BitFinite Node software a maximum of 1000 bytes are
...
A C++ reference implementation is available in BitFinite Node
...
This specification is based on an adaptation by BitFinite Node of a
...
branch construction in the BitFinite Node source code [4].
