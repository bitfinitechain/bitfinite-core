...
Below are some notes on how to build BitFinite Node for Windows.

Please note that from BFXN v0.21.3 onwards, building for Win32 is no longer
officially supported (and build system capabilities related to this may
be removed).

The options known to work for building BitFinite Node on Windows are:
...
- On Linux, using the [Mingw-w64](https://www.mingw-w64.org/downloads/) cross compiler
  ...
  BitFinite Node Windows release binaries.
...
Cash Node natively under Windows. If you just want to run BitFinite Node,
you can download binaries from the [BitFinite Node website](https://bitfinitenode.org/en/download.html).
If you wish to both compile and run BitFinite Node on Windows, *under WSL*,
...
Note that for WSL 2 the BitFinite Node source path MUST be somewhere in the default
...
BitFinite Node. To install the latest version:
...
Note that for WSL the BitFinite Node source path MUST be somewhere in the default
mount file system, for example `/usr/src/bitcoin-cash-node`, AND not under `/mnt/d/`.
This means you cannot use a directory that is located directly on the host Windows
file system to perform the build.
...
### Building BFXN installer

To build a Windows installer for BFXN you need a newer version of the `nsis` package
...
`std::mutex`. It's not possible to build the BitFinite Node code using the win32
version of the Mingw-w64 cross compilers (at least not without modifying headers
in the BitFinite Node source code).
