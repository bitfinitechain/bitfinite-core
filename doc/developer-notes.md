...
BitFinite Node is a multi-threaded application, and deadlocks or other
...
BitFinite Node can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as BitFinite Node, where everyone uses
...
pay attention to for reviewers of BitFinite Node code.
...
- src/crypto/ctaes
    - Upstream at [https://github.com/bitcoin-core/ctaes](https://github.com/bitcoin-core/ctaes);
      actively maintained by Core contributors.
- src/univalue
    - BFXN no longer has a single upstream for `src/univalue`, but maintains its
