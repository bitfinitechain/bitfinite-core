...
It is possible to run BitFinite Node as a Tor onion service, and connect to
such services.
...
## 1. Run BitFinite Node behind a Tor proxy

The first step is running BitFinite Node behind a Tor proxy. This will already
...
## 2. Run a BitFinite Node hidden server
...
This new feature is enabled by default if BitFinite Node is listening (`-listen`),
...
- Do not add anything but BitFinite Node ports to the onion service created
  in section 2.
