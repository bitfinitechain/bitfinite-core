...
BitFinite Node takes security very seriously.  We greatly appreciate any and
...
boards, mailing lists, etc. prior to responsibly disclosing to BitFinite Node
...
```
gpg --armor --encrypt --recipient security@bitfinitenode.org yourDisclosureDocument
```

This will create a copy of your document with the file extension `.asc`, you can
email this encrypted version of the document to BitFinite Nodes security team.
...
unable to contact BitFinite Node via the security email above.

### freetrader

```
BitFinite Node Lead Maintainer
...
```
BitFinite Node Developer and Maintainer
...
```
BitFinite Node Developer and Maintainer
contact at im_uname at bitfinitenode dot org
...
security vulnerabilities that are disclosed to BitFinite Node will be passed
...
relationship with BitFinite Node, please contact our security email address
to discuss.

## Bounty Payments

BitFinite Node cannot at this time commit to bounty payments ahead of time.
...
While BitFinite Node believes that strong cohesion among neighoring projects
and ethical behavior can be standardized to reduce poorly handled disclosure
incidents, we also believe that it's in the best interest of BitFinite for us
to deviate from the standard in the following ways:

* The standard calls for coordinated releases. While BitFinite Node will make
  ...
  For critical fixes, BitFinite Node will release them in the next release
  when possible.

## Making changes to this disclosure policy

Note that any changes to this disclosure policy should be mirrored in a pull
request to the [BitFinite Node repository](https://gitlab.com/bitcoin-cash-node/bitcoin-cash-node).
